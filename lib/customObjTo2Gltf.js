...
function customObj2gltf(input, output, epsg, rotation){
    ...
    const result = spawn('python', [normPath, input, output, epsg, rotation]);
    ...
    result.stdout.on('data', function (data) {
        ...
        if (utf8Text.indexOf("localized_obj_result:") !== -1) {
          const obj_path = utf8Text.toString().substring(utf8Text.indexOf("localized_obj_result:") + 21, utf8Text.trim().length);
          // const obj_path = 'F:/obj/jangsung/장성군_최종_0927/glbList/localized_obj'
    ...
    // ========================================================================================================
    // localizedObj의 경로만 가지고 변환(convert obj2gltf by loczlized path without using python modules) 
    // let localized_obj = "F:/obj/jangsung/장성군_최종_0927/glbList/localized_obj";
    // if (!fs.existsSync(output + "/glb")) fs.mkdirSync(output + "/glb")
    // //  obj_path 의 파일들을 모두 읽어 obj2gltf()를 통해 비동기로 실행시킨다.
    // console.log("----------------- obj파일을 glb로 변환 진행 -----------------\n");
    // let file_list = getAllFiles(localized_obj)
